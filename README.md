...
这个小部分，我解决了这样子的一些问题：就是很多人反馈的哪个前端部分按钮没办法正常响应的问题
- 点击http://localhost:8080/chat 路由当中那个我的分享按钮产生的弹窗当中编辑和删除按钮没办法正常使用的问题
- 点击会话列表种的分享按钮，输入哪个时间和测试后，它创建按钮点击后，没有提示并关闭弹窗的问题
...
在这个小部分当中，我实现了那个管理员的后端函数，然后的话，那个管理用户的页面也实现一部分，只不过，那个页面有一些问题，像是那个最后登录、注册时间、管理员的角色显示存在一些问题，只不过，应该可以改好

# v -3.65

在这个小部分当中，我修改了这么一些问题：
- 注册时间展示出错
- 个角色那一列没办法正常使用，连管理员admin都变为那个普通用户
- 管理员只能创建普通用户
- 最后登录时间正常显示
...
这个小部分当中，我实现了这样一些东西：
- web/root/index,html的那个更新
- 那个会话管理、笔记管理的那个页面
- 用户管理功能 、 会话管理功能那几个数据库函数的测试
- 管理员用户数据库函数 、 管理员会话管理数据库函数 、 管理员笔记功能数据库函数的测试